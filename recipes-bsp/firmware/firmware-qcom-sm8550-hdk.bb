...
do_install:append() {
    if [ -n "${ADRENO_URI}" ] ; then
        install -m 0644 ${WORKDIR}/adreno/${ADRENO_PATH}/a740_sqe.fw ${D}${FW_QCOM_BASE_PATH}
        install -m 0644 ${WORKDIR}/adreno/${ADRENO_PATH}/gmu_gen70200.bin ${D}${FW_QCOM_BASE_PATH}
    fi
}
